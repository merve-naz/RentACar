...
public class PaymentsController {
	...
	@GetMapping
	public ResponseEntity<?> getAll() {//ResponseEntity sınıfı :HttpEntity sınıfına ek olarak builder tasarım desenini kullanarak isteğe yanıt olarak header bilgisi, HTTP durum kodu gibi bilgileri eklemeyi sağlar. 
		DataResult<List<GetAllPaymentsResponse>> result = paymentService.getAll();
		if (result.isSuccess()) {
			return ResponseEntity.ok(result);//responseEntitiy nesnesi döndururuz
	...
	@PostMapping
	public ResponseEntity<?> add(@Valid @RequestBody CreatePaymentRequest request) {//payment bilgilerini ekle
