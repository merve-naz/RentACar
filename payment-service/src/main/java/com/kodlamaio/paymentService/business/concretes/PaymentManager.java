...
public class PaymentManager implements PaymentService {
	...
	public Result checkIfPaymentSuccessful(CreateRentalPaymentRequest request) {//
		checkPayment(request);//boyle bir Kredi Kart bilgileri var mı?
		return new SuccessResult(Messages.PaymentSuccessful);//odeme basarılı
	}

	private void checkPayment(CreateRentalPaymentRequest request) {//payment kontrol et _____ boyle bir kart bilgisi var mı? varsa da paradan eksilt
		
		if (!paymentRepository.existsByCardNumberAndFullNameAndCardExpirationYearAndCardExpirationMonthAndCardCvv(//böyle blr card number ad,cvv yeay.ay var mı diye bak
			...
			Payment payment = paymentRepository.findByCardNumber(request.getCardNumber());//payment repository'den o kart numarasını bul.(pk) ıd bakamazsın.
