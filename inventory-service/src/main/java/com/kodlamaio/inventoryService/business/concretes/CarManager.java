...
public class CarManager implements CarService {
	...
	@Override
	public Result checkCarAvailable(String carId) {//araba var mı yok mu diye bakıp varsa state durumuna bakıyor
		checkIfCarNotExistsById(carId);//araba var mı diye bak(carId aslında car repository'nin ıdsidir)
		Car car = this.carRepository.findById(carId).get();
		if (car.getState() != 1) {//state 1 demek araç kiralandı demek
	...
	private void checkIfCarNotExistsById(String id) {
		if (!this.carRepository.findById(id).isPresent()) {//carRepository'de bu id'ye(pk) sahip bir araba var mı yok mu
